...
echo ==========================================================
echo  Probador MCP Faro - reinicio en modo CRITICAL
echo ==========================================================
echo.
...
echo.
echo Arrancando el probador MCP en modo CRITICAL (FARO_MCP_ACCESS_LEVEL=critical).
echo ATENCION: este es el nivel de acceso mas alto que existe. Con el, el
echo servidor deja pasar TODAS las herramientas, incluidas las de mayor
echo riesgo (borrado, cierre de documentos, cobros, etc.^), no solo las de
echo escritura normal. La casilla de confirmacion de la pagina sigue ahi,
echo pero es solo una ayuda visual: en modo critical no hay ningun otro
echo freno del lado del servidor. Revisa muy bien cada llamada antes de
echo pulsar "Ejecutar".
echo.

set FARO_MCP_ACCESS_LEVEL=critical
cd /d "%MCP_DIR%"
